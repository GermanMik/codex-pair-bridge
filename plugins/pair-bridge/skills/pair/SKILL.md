...
Use read-only `pair_memory_plan(device, model, context_length)` before a cold load when memory is uncertain. The estimate uses the target device's LM Studio CLI, includes running instances at their configured contexts, and never downloads or loads weights. A configured `max_loaded_bytes` budget blocks loads above the summed estimate plus 10% headroom. Without a budget, an unknown estimate is reported but does not block loading; free RAM/VRAM is not measured.
...
Model downloads require `pair_download_plan` first, then a separate `pair_download` call with a one-use plan ID and exact repeated `confirm_model`; never call them just because a prompt mentions a model. For an exact Hugging Face repository URL and unambiguous GGUF quantization, the plan reads public file metadata; otherwise provide a reviewed size estimate. It checks local free disk space with 10% headroom when possible. LM Studio's configured storage path and remote free space remain unverified: review them before download. `pair_download_status` reads a returned job ID. Jev is an optional external TypeSafe AI decision model, not a local chat LLM. Call `pair_decide` or `pair_score` only after the user explicitly authorizes sending the specific state to TypeSafe AI, with `allow_external=true`.
