...
- [ ] Replace the weight-size-only memory gate with a per-device preflight. Read `loaded_instances[].config.context_length` for running instances; never mistake `max_context_length` for the configured context. For an unloaded installed model, choose an explicit planned context (and GPU offload policy), then run `lms load --estimate-only --context-length N` on the target device. Compare estimated total/GPU memory with available capacity and existing loads, retain a safety margin, and mark estimates as estimates. If the CLI is unavailable, report unknown rather than silently treating `size_bytes` as RAM use. A one-at-a-time real-load calibration may be optional later; do not load every model merely to discover memory use. [LM Studio CLI](https://lmstudio.ai/docs/cli/local-models/load), [model inventory](https://lmstudio.ai/docs/developer/rest/list).
- [ ] Keep model download as a **separate explicit action**, never part of ask. `pair_download_plan` currently displays caller-supplied size/destination before a one-use `pair_download` call. Improve the plan by resolving exact repository, revision and quantization, verifying file-size metadata and destination/free space where possible, and clearly marking unknown values. LM Studio's download API reports `total_size_bytes` only after the job starts; do not start a download just to learn its size. Retain explicit model confirmation and progress/status reporting. [LM Studio download API](https://lmstudio.ai/docs/developer/rest/download).
